...
const checkSearch = async (data, msgIdSet) => {
  ...
  try {
    ...
    if (lat && long && area_code) {
      ...
      searchObj["RGC-start-Err"] = `Start location GPS or area code is missing`;
    }
  } catch (error) {
  ...
  console.log("Checking Reverse Geocoding for `end` location in `fulfillment`");
  try {
    if (endLocation) {
      ...
      if (lat && long && area_code) {
        ...
        searchObj["RGC-end-Err"] = `End location GPS or area code is missing`;
      }
    } else {
      searchObj["RGC-end-Err"] = `End location GPS or area code is missing`;
    }
