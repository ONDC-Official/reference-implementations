...
const checkConfirm = (data, msgIdSet) => {
  ...
  if (!bppTermsTag) {
    console.log("BPP_Terms not found in onInitTags");
    return;
  }
  ...
  try {
    ...
    searchTags.forEach((searchTag) => {
      const matchingConfirmTag = confirmTags.find(
        (confirmTag) => confirmTag.descriptor.code === searchTag.descriptor.code
      );
      ...
      searchTag.list.forEach((searchListItem, index) => {
        ...
        if (!_.isEqual(searchListItem.descriptor, confirmListItem.descriptor)) {
          confirmObj[
